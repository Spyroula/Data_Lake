...
def process_song_data(spark, input_data, output_data):
    """
    Process all the songs data JSON files in the given input folder, 
    extracts songs and artists tables and stores them in data files 
    in parquet format in the given output folder.
    
    Parameters
    ----------
    :param spark: a spark session instance
    :param input_data: the input file path
    :param output_data: the output file path
    ...
    artists_table = df.select(artists_columns).drop_duplicates()
...
def process_log_data(spark, input_data, output_data):
    ...
    Process all log data JSON files in the given input folder, extracts data for  the table time,
    users and songplays and stores them in data files in parquet format in the given output folder.
    ...
    users_table = df.select(users_columns).drop_duplicates()
    ...
    time_table = df.withColumn("hour",hour("start_time"))\
                    ...
                    .select("ts","start_time","hour", "day", "week", "month", "year",
                            "weekday").drop_duplicates()
    
    # write time table to parquet files partitioned by year and month
    time_table.write.parquet(os.path.join(output_data, "time_table/"), partitionBy=["year","month"], mode='overwrite')
    ...
    .select(monotonically_increasing_id().alias("songplay_id"),col("start_time"),col("userId")\
            .alias("user_id"),"level","song_id","artist_id", col("sessionId")\
    ...
    songplays_table.drop_duplicates().write.parquet(os.path.join(output_data, "songplays/"), partitionBy =["year","month"], mode="overwrite" )
...
def main():
    ...
    output_data = "s3a://sparkify-data-lake-udend/"
